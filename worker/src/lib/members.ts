...
export const MEMBER_EXPORT_COLUMNS: CsvColumn<UserRecord>[] = [
  ...
  { key: "qoj", label: "QOJ", value: (u) => u.qoj },
  { key: "jungol", label: "정올", value: (u) => u.jungol },
...
export async function createUser(env: Env, input: UserInput): Promise<UserRecord> {
  ...
  const result = await env.CONTENT_DB.prepare(
    `INSERT INTO users (uid, discord_id, name, email, student_id, phone, solved_ac, codeforces, atcoder, doj, qoj, jungol, avatar_url, nickname)
...
export async function upsertUserByDiscordId(
  ...
  if (input.qoj !== undefined) add("qoj", input.qoj);
  if (input.jungol !== undefined) add("jungol", input.jungol);
...
export async function updateUser(env: Env, uid: string, input: UserUpdateInput): Promise<void> {
  ...
  await env.CONTENT_DB.prepare(
    `UPDATE users SET discord_id=?2, name=?3, email=?4, student_id=?5, phone=?6, solved_ac=?7, codeforces=?8, atcoder=?9, doj=?10, qoj=?11, jungol=?12, nickname=?13, updated_at=datetime('now')
     WHERE uid=?1`,
  )
    .bind(
      uid,
...
// 달리, 세션으로 이미 확인된 본인 uid에 대해 핸들들만 딱 고칩니다. 이름/이메일/
...
export async function updateUserHandles(env: Env, uid: string, input: OwnHandlesInput): Promise<void> {
  await env.CONTENT_DB.prepare(
    `UPDATE users SET solved_ac=?2, codeforces=?3, atcoder=?4, doj=?5, qoj=?6, jungol=?7, updated_at=datetime('now') WHERE uid=?1`,
  )
    .bind(uid, input.solvedAc, input.codeforces, input.atcoder, input.doj, input.qoj, input.jungol)
    .run();
}
...
export type HandleSite = "solvedAc" | "codeforces" | "atcoder" | "doj" | "qoj" | "jungol";
...
const HANDLE_COLUMN: Record<HandleSite, string> = {
  solvedAc: "solved_ac",
